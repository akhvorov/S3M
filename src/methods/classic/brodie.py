...
from methods.classic.tfidf import TfIdfModule
...
class BrodieModel(SimStackHyperoptModel):
    def __init__(self, coder: SeqCoder, d: float = -1.0):
        self.tfidf = TfIdfModule(coder)
        self.d = d

    def params_edges(self) -> Dict[str, Tuple[float, float]]:
        return {"d": (-1, 0)}

    def fit(self, sim_train_data: List[Tuple[int, int, int]] = None, unsup_data: List[int] = None) -> 'BrodieModel':
    ...
    def predict(self, anchor_id: int, stack_ids: List[int]) -> List[float]:
        scores = []
        anchor = self.tfidf.coder.to_seq(anchor_id)
        anchor_tfidf = self.tfidf.tfidf_vectorizer.transform(anchor_id)
        anchor_tfidf = {k: 1 - math.exp(1 - idf) for k, (_, idf) in anchor_tfidf.items()}
        for stack_id in stack_ids:
            stack = self.tfidf.coder.to_seq(stack_id)
    ...
    def name(self) -> str:
        return self.tfidf.name() + f"_brodie_{self.d}"
